...
public class OauthAuthenticateValidatorCallbackHandler implements AuthenticateCallbackHandler {
    ...
    public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        for (Callback callback : callbacks) {
            ...
            //log.info("callback.toString "+callback.toString());
    ...
    private void handleCallback(OAuthBearerValidatorCallback callback){
        ...
        //String refreshToken = callback.refreshTokenValue();
        if (accessToken == null) {
            log.info("***** accessToken == null");
            throw new IllegalArgumentException("Callback missing required token value");
        }

        OauthBearerTokenJwt token = OauthHttpCalls.introspectBearer(accessToken);//ok
        ...
        if(now > token.expirationTime()){
            //OauthHttpCalls.refresh(refreshToken);
